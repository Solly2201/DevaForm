...
/**
 * How far the camera still is from the hero angle, published for the
 * backdrop behind the canvas to read.
 *
 * The backdrop is a room seen from one point. This is the one number that
 * says how true that still is right now, and it lives in a store because
 * the thing that needs it is a DOM layer rather than part of the scene.
 */
export function StageAlignment({
  camera: hero,
  recedeWithin,
}: {
  camera: StageCamera;
  ...
  const camera = useThree((state) => state.camera);
  const setAlignment = useStageStore((state) => state.setAlignment);
  const phase = useStageStore((state) => state.phase);

  const heroAzimuth = useRef(0);
  const heroDistance = useRef(1);
  useEffect(() => {
    const target = new THREE.Vector3(...hero.target);
    const radius = new THREE.Vector3(...hero.position).sub(target);
    heroAzimuth.current = Math.atan2(radius.x, radius.z);
    heroDistance.current = radius.length();
  }, [hero]);

  useFrame(() => {
    // While the sequence runs the room is simply there; it starts
    // answering to the camera once the customer has the controls.
    if (phase !== "ready") {
      setAlignment(1);
      return;
    }
    const target = new THREE.Vector3(...hero.target);
    const radius = camera.position.clone().sub(target);
    const azimuth = Math.atan2(radius.x, radius.z);
    let turned = Math.abs(azimuth - heroAzimuth.current);
    if (turned > Math.PI) turned = Math.PI * 2 - turned;
    const byAngle = 1 - Math.min(1, turned / recedeWithin);
    // Moving IN is fine — the room is behind the statue either way. Moving
    // far out shows the hall's own edges, so it goes with distance too.
    const pulled = Math.max(0, camera.position.distanceTo(target) / heroDistance.current - 1.35);
    const byDistance = 1 - Math.min(1, pulled / 0.5);
    setAlignment(Math.min(byAngle, byDistance));
  });
